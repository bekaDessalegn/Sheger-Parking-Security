...
class _ExpectedPageState extends State<ExpectedPage> {
  ...
  Future editParked() async {
    ...
    var url = Uri.parse('http://192.168.1.6:5000/token:qwhu67fv56frt5drfx45e/reservations/$reserveId');
  ...
  Widget buildReservation(ReservationDetails reservationDetail) => GestureDetector(
    onTap: () {
      Navigator.push(
          context,
          MaterialPageRoute(
              builder: (context) => ClientReservationDetails(reservationId: reservationDetail.id, client: reservationDetail.client, reservationPlateNumber: reservationDetail.reservationPlateNumber, branch: reservationDetail.branch, slot: reservationDetail.slot, price: reservationDetail.price.toString(), startingTime: reservationDetail.startingTime.toString(), duration: reservationDetail.duration.toString(), parked: reservationDetail.parked,)));
    },
    child: Padding(
